...
public class AlarmService {
    ...
    @Autowired
    public void setFeignAlarmService(FeignAlarmService feignAlarmService) {
    ...
    private void pushAlarm(AlarmDto alarm) {
        try {
            if (feignPushEnable) {
                logger.debug("使用Feign API创建告警. {}", alarm);
                feignAlarmService.newAlarm(alarm);
            } else if (restfulPushEnable) {
